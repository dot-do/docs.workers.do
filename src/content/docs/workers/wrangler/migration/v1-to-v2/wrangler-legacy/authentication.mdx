...
```sh
CF_ACCOUNT_ID=accountID CF_API_TOKEN=veryLongAPIToken mdxe run worker.mdx --deploy
```

Alternatively, you may use the `CF_EMAIL` and `CF_API_KEY` environment variable combination instead:

```sh
CF_EMAIL=cloudflareEmail CF_API_KEY=veryLongAPI mdxe run worker.mdx --deploy
```
