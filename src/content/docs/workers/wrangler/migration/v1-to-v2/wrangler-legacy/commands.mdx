...
```sh
mdxe create worker.mdx [$NAME] [--type=$TYPE] [--site]
```
...
```sh
mdxe login [--scopes-list] [--scopes $SCOPES]
```
...
```sh
mdxe run worker.mdx --deploy [--env $ENVIRONMENT_NAME]
```
...
```sh
mdxe run worker.mdx --dev [--env $ENVIRONMENT_NAME] [--ip <ip>] [--port <port>] [--host <host>] [--local-protocol <http|https>] [--upstream-protocol <http|https>]
```
...
```sh
mdxe tail worker.mdx [--format $FORMAT] [--status $STATUS] [OPTIONS]
```
