...
For more detailed information on configuring your Worker (for example, if you are using [jurisdictions](/r2/reference/data-location/#jurisdictional-restrictions)), refer to the [Wrangler Configuration documentation](/workers/wrangler/configuration/).
...
```sh
pnpm --filter mdxe exec mdxe run worker.mdx --deploy
```
